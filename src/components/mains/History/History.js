...
function OrderHistory() {
    ...
    const success = (<Progress type="circle" className='s-success' percent={100} width={24} />);
    const cancel = (<Progress type="circle" className='f-cancel' percent={80} width={24} status="exception" />);
    ...
    useEffect(() => {
        if(token){
            const getHistory = async() =>{
                const res = await axios.get('http://localhost:3001/private/api/v1/order/gethistory', {
        ...
        <div className='Orders-page'>
            ...
            { history === null ? null : history.map(e => (
                ...
                <Space direction="vertical">
                    ...
                    <div className="spin-ez">{e.status === 'PROCESS'?antIcon:e.status === 'COMPLETED'?success:e.status === 'CANCEL'?cancel:'null'} {e.status === 'PROCESS'?' In Process':e.status === 'COMPLETED'?'Completed':e.status === 'CANCEL'?"Cancel":'null'}</div>
